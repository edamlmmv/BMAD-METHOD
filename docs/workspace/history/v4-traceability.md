...
| --- | --- | --- | --- |
| S25 CLI is Session-only | Done | `a1c161dfb`, `test/test-workspace-cli.js` | Help omits `--mission-id`. |
| S26 Runtime path is Session-only | Done | `a1c161dfb`, `tools/workspace/` | Runtime uses `sessions/<sessionId>`. |
| S27 Packet file is canonical | Done | `a1c161dfb`, `tools/workspace/packet.js` | Packet writes `bmad-work-packet.json`. |
| S28 V4 packet schema is enforced | Done | `a1c161dfb`, `test/test-workspace-contracts.js` | Legacy fields fail. |
| S29 Setup Gate blocks silent packets | Done | `a1c161dfb`, `test/test-workspace-cli.js` | Setup refs or skips required. |
| S30 Matt skill snapshots are pinned | Done | `a1c161dfb`, `docs/workspace/vendor/mattpocock-skills/` | Manifest and checksums validate. |
| S31 Base Improvement Session Kit is canonical | Done | `a1c161dfb`, `docs/workspace/templates/` | Self-improvement template names removed from runtime templates. |
| S32 V4 progress is traceable | Done | `a1c161dfb`, `docs/workspace/v4-traceability.md` | Acceptance mapping exists. |
