---
title: "BMAD Workspace V1 Implementation Backlog"
description: TDD-first epics and stories for the BMAD Workspace V1 slice
---

# BMAD Workspace V1 Implementation Backlog
...
- `docs/workspace/capability-contract.md`
- `docs/workspace/adr-candidates.md`
- `docs/workspace/v1-acceptance-tests.md`
...
- Base Mutation Grant as the only path to BMAD Workspace writes.
...
- auto-promotion into the BMAD Workspace
...
| E5 Workspace Skill Boundary | Decide focused skill boundary. | AT-030, AT-032 | E1 | Replacing MissionCTL harness | Graphify leaking into BMAD truth. |

## Stories

| ID | Story | Public Surface | Likely Files Or Areas | Red Test | Green Target | Acceptance Mapping |
| --- | --- | --- | --- | --- | --- | --- |
| S1 | Validate BMAD Workspace artifact contracts. | `node test/test-workspace-contracts.js` | `test/`, `tools/workspace/` | Invalid packet without acceptance criteria fails. | Minimal contract validator for packet and capability fixtures. | AT-020, AT-022, AT-030 |
...
| S10 | Block BMAD Workspace writes without grant. | Workspace commands through Grant Guard | Grant Guard | Normal mission can write base path. | Base mutation attempt exits nonzero and records violation artifact. | AT-031, AT-041, AT-060 |
...
- BMAD Workspace Git diff remains clean except intentional implementation files
...
| T3 | Launch mutates BMAD Workspace unexpectedly. | Write mission state only under configured runtime path. |
...
- BMAD Workspace can become a broad platform. V1 must stay CLI, files, and Git.
...
Create `bmad-workspace` after this backlog is accepted. The new skill
should own BMAD Workspace planning, contracts, acceptance mapping, Base
